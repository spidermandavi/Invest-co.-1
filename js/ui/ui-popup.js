...
// ===== PLAYER WORTH COMPARISON =====
function showComparisonPopup() {
  if (!players || !stocks) return;

  let html = "<b>Player Worth Comparison</b><br><br>";
  players.forEach((p, i) => {
    let totalWorth = p.money;
    stocks.forEach(s => totalWorth += (s.owned[i] || 0) * s.price);
  ...
  // Draw combined graph after popup renders
  setTimeout(() => {
    ...
    // Find min/max across all player histories
...
window.showComparisonPopup = showComparisonPopup;
